...
# If running as root, execute as ec2-user
if [ "$(whoami)" != "ec2-user" ]; then
    echo "Not ec2-user. Re-executing as ec2-user..."
    
    # Make sure ec2-user has access to the script
    SCRIPT_PATH="$0"
    chown ec2-user:ec2-user "$SCRIPT_PATH"
    chmod +x "$SCRIPT_PATH"
    
    # Switch to ec2-user with full login shell
    exec runuser -l ec2-user -c "$SCRIPT_PATH"
fi

# From here, running as ec2-user
AWS_REGION="ap-south-1"
ECR_REGISTRY="384570460482.dkr.ecr.${AWS_REGION}.amazonaws.com"
KIND_IMAGE="${ECR_REGISTRY}/kindest-node:v1.34.3"
CLUSTER_NAME="dev"
...
echo "📦 Installing tools..."
/home/ec2-user/automation-platform/shell-scripts/kind/kind-installation.sh
